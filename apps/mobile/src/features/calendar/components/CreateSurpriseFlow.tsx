...
export function CreateSurpriseFlow({
  ...
  const [category, setCategory] = useState<'cena' | 'regalo' | 'flores' | 'escapada' | 'plan_juntos' | 'carta' | 'especial'>('cena');
  const [title, setTitle] = useState('');
  const [date, setDate] = useState('2026-08-29');
  const [time, setTime] = useState('21:00');
  const [location, setLocation] = useState('');
  const [revealOption, setRevealOption] = useState<'now' | 'one_day_before' | 'same_day_morning' | 'specific_time' | 'manual'>('one_day_before');
        ...
        <View style={styles.sheet} onStartShouldSetResponder={() => true}>
          ...
          <ScrollView style={styles.scrollBody} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.visibilityRow}>
              {[
                { id: 'total_secret', label: '🔒 Totalmente secreto', desc: 'Solo alerta general' },
