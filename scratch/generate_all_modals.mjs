...
// ── 1. apps/mobile/src/components/map/PlaceGalleryModal.tsx ──
const placeGalleryModalTs = `import React, { useState } from 'react';
import {
  View,
  Text,
  StyleSheet,
...
import { IconX, IconPlus, IconSparkles } from '../ui/Icons';
...
export function PlaceGalleryModal({
  ...
  // Extract all photos (combine imageUrl and photos array)
          ...
          >
            ...
            <View style={styles.uploadCard}>
              ...
              <Text style={styles.uploadCardSubtitle}>
                Sube fotos de tus momentos aquí para que Tonet y Andrea las tengan siempre
              </Text>
              <PhotoUploadField
                photoUrl={null}
...
`;
fs.writeFileSync(path.join(mobileRoot, 'src', 'components', 'map', 'PlaceGalleryModal.tsx'), placeGalleryModalTs, 'utf8');

// ── 2. apps/mobile/src/components/map/PlaceDetailModal.tsx ──
const placeDetailModalTs = `import React from 'react';
import {
  View,
  Text,
  StyleSheet,
...
import { IconMapPin, IconCalendar, IconHeart, IconUtensils, IconSparkles, IconEdit } from '../ui/Icons';
      ...
      <View style={styles.overlay}>
        <View style={styles.sheetContainer}>
          ...
          <ScrollView style={styles.scrollArea} showsVerticalScrollIndicator={false}>
            ...
            {/* 2. CITA (Doble destino y quién invitó) */}
            {place.type === 'date' && (
              <View style={styles.infoCard}>
            ...
            {/* 3. VIAJE (Alojamiento, días, sitios visitados) */}
            {place.type === 'trip' && (
              <View style={styles.infoCard}>
...
`;
fs.writeFileSync(path.join(mobileRoot, 'src', 'components', 'map', 'PlaceDetailModal.tsx'), placeDetailModalTs, 'utf8');

// ── 3. apps/mobile/src/components/map/MapBottomSheet.tsx ──
const mapBottomSheetTs = `import React from 'react';
import {
  View,
  Text,
  Image,
  ...
  return (
    <View style={styles.sheetContainer} pointerEvents="box-none">
      <View style={styles.card}>
        ...
        {/* Action Buttons Row (3 Actions: Ver Galería, Detalle Completo, Editar) */}
...
`;
fs.writeFileSync(path.join(mobileRoot, 'src', 'components', 'map', 'MapBottomSheet.tsx'), mapBottomSheetTs, 'utf8');

// ── 4. apps/mobile/src/components/map/AddPlaceLocationModal.tsx ──
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.sheetCard}>
          ...
          {/* STEP 3: DETAILS WITH TYPE SPECIFIC FIELDS */}
          {step === 'details' && (
...
// ── 5. apps/mobile/app/(tabs)/map/index.tsx ──
...
export default function MapScreen() {
  ...
  // Open expanded full-screen gallery
  ...
  return (
    <View style={styles.container}>
      ...
      {/* 5. Bottom Sheet with quick actions: Ver Galería, Detalle, Editar */}
      <MapBottomSheet
        place={selectedPlace}
        group={selectedGroup}
      ...
      {/* 6. Full-Screen Rich Place Detail Modal */}
      <PlaceDetailModal
      ...
      {/* 7. Full-Screen Expanded Gallery Modal */}
      <PlaceGalleryModal
      ...
      {/* 8. Add / Edit Place Location Modal */}
...
console.log('✅ Full implementation of PlaceDetailModal, PlaceGalleryModal, MapBottomSheet, and AddPlaceLocationModal complete.');
`;

fs.writeFileSync(path.join(projectRoot, 'scratch', 'generate_all_modals.mjs'), allModalsScript, 'utf8');
