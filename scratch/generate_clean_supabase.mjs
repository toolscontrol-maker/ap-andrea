...
console.log('--- 1. Generating Supabase config & migrations ---');
const supabaseDir = path.join(projectRoot, 'supabase');
const migrationsDir = path.join(supabaseDir, 'migrations');
ensureDir(migrationsDir);

// 1.1 config.toml
fs.writeFileSync(path.join(supabaseDir, 'config.toml'), `[api]
enabled = true
port = 54321
...
// 1.2 0001_initial_schema.sql
fs.writeFileSync(path.join(migrationsDir, '0001_initial_schema.sql'), `-- ==============================================================================
...
// 1.3 0002_rls_policies.sql
fs.writeFileSync(path.join(migrationsDir, '0002_rls_policies.sql'), `-- ==============================================================================
...
// 1.4 0003_realtime.sql
fs.writeFileSync(path.join(migrationsDir, '0003_realtime.sql'), `-- ==============================================================================
-- 0003_realtime.sql
...
// 1.5 0004_storage.sql
fs.writeFileSync(path.join(migrationsDir, '0004_storage.sql'), `-- ==============================================================================
-- 0004_storage.sql
...
// 1.6 seed.sql
fs.writeFileSync(path.join(supabaseDir, 'seed.sql'), `-- ==============================================================================
-- seed.sql
-- Development seed template (Clean - no production dummy data).
-- ==============================================================================
`, 'utf8');

console.log('✅ Supabase migrations & config written.');
...
fs.writeFileSync(path.join(libDir, 'supabase.ts'), `import { createClient } from '@supabase/supabase-js';
...
console.log('✅ src/lib/supabase.ts written.');
...
fs.writeFileSync(path.join(repoDir, 'AndreaRepository.ts'), `import { WishlistItem, Place, CoupleEvent, RitualSeed, FeelingEntry } from '@andrea/types';
...
fs.writeFileSync(path.join(repoDir, 'LocalAndreaRepository.ts'), `import { WishlistItem, Place, CoupleEvent, RitualSeed, FeelingEntry } from '@andrea/types';
import { AndreaMapPlace } from '../types/map';
...
fs.writeFileSync(path.join(repoDir, 'SupabaseAndreaRepository.ts'), `import { WishlistItem, Place, CoupleEvent, RitualSeed, FeelingEntry } from '@andrea/types';
import { AndreaMapPlace } from '../types/map';
...
fs.writeFileSync(path.join(repoDir, 'index.ts'), `import { AndreaRepository } from './AndreaRepository';
import { defaultLocalRepository } from './LocalAndreaRepository';
...
console.log('✅ Repositories written.');
...
fs.writeFileSync(path.join(provDir, 'AuthProvider.tsx'), `import React, { createContext, useContext, useEffect, useState, ReactNode } from 'react';
...
fs.writeFileSync(path.join(provDir, 'CoupleProvider.tsx'), `import React, { createContext, useContext, useEffect, useState, ReactNode } from 'react';
import { supabase, isSupabaseConfigured } from '../lib/supabase';
...
fs.writeFileSync(path.join(provDir, 'RealtimeSyncProvider.tsx'), `import React, { createContext, useContext, useEffect, useState, ReactNode } from 'react';
import { supabase, isSupabaseConfigured } from '../lib/supabase';
...
fs.writeFileSync(path.join(provDir, 'index.ts'), `export * from './AuthProvider';
...
fs.writeFileSync(path.join(authFeatureDir, 'auth.types.ts'), `export interface LoginCredentials {
  email: string;
  password: string;
}
...
fs.writeFileSync(path.join(authFeatureDir, 'LoginScreen.tsx'), `import React, { useState } from 'react';
...
fs.writeFileSync(path.join(authFeatureDir, 'RegisterScreen.tsx'), `import React, { useState } from 'react';
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ActivityIndicator, Alert, KeyboardAvoidingView, Platform } from 'react-native';
import { useAuth } from '../../providers/AuthProvider';
import { Colors } from '../../theme/colors';
import { Typography, Spacing, Radii } from '../../theme/tokens';
...
fs.writeFileSync(path.join(authFeatureDir, 'PairingScreen.tsx'), `import React, { useState } from 'react';
...
console.log('✅ Auth & Pairing features written.');

// ─────────────────────────────────────────────────────────────────────────────
// 6. ENVIRONMENT TEMPLATE: .env.example & RLS TEST SCENARIOS
// ─────────────────────────────────────────────────────────────────────────────
...
fs.writeFileSync(path.join(scratchDir, 'test_rls_scenarios.sql'), `-- ==============================================================================
...
console.log('✅ .env.example and test_rls_scenarios.sql written.');
